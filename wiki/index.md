...
> 最后更新：2026-09-18（**派生登记 | AUVNetModem**：面向五U 组网条件的水声通信机详细设计（TechReq，engineering-hardware 子型第二例，A 算法 + B 硬件，依赖 AUVNetCoop / UWAcomm / AUVProposal，git `c37e2ce`，ADR-050）——用户裁通信机详细设计从 AUVNetCoop 单列、范围 A + B；两路只读采集沉淀项目 wiki 2 页（既有资产全为 8–16 kHz 口径 / 两型 U 无硬件事实 / 平台发射 3～6 kHz 与五U 3–9 kHz 有出入），SPEC-001 框架 v0 待裁 D1~D10；登记面当日全量 + CANON 级联（活跃 37→38 / TechReq×8→×9 / ADR ~049→~050 / memory 112→115（project 73→74；另补登 09-18 上一会话两条 PowerShell feedback 35→37）/ `MEMORY.md` 索引 →116 行）；后台会话在分支 `reg/auvnetmodem` 登记，待 ff-merge；页面总数不变）
> 页面总数：113
...
- [harness-resources](topics/harness-resources.md) — Claude Code Harness 全景索引（Hooks 9 + Skills 90+ + Rules + Agents + 6 MCP），加载层次决策树（2026-06-09 三模板来源口径同步）
- [memory-index](topics/memory-index.md) — `~/.claude/projects/D--Claude/memory/` 125 条 auto-memory 按类型 + 主题聚合索引（2026-05-24 新建）
...
- [ai-memory-system-design-framework](source-summaries/ai-memory-system-design-framework.md) — AI 记忆系统设计框架笔记：开放任务 = 上下文差 → 学写入策略；惊讶写入 / 效用留存 / 规则+例外表 / 遗忘即功能；六模块（装配器/显著性门/情景库/巩固进程/语义库/程序库）+ 四不变量（I1 存索分离 / I2 provenance / I3 两道门 / I4 读取即投票）；附本体系五层栈逐层对照（I4 缺口 + I1 部分违反），2026-08-30 ingest
