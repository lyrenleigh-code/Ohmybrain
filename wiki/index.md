...
> 最后更新：2026-08-29（**入会审计（十七，用户显式要求）**：距十六 5 天。① **CANON 级联收口 14 处**（「审计后窗口」反模式**第 5 次复发**）：08-26 AUVProposal session 当日新增 2 条 feedback memory（`bash_heredoc_backslash` / `path_form_vs_location`）漏级联 + 本 session 新装 skill `archify` → **memory 97→99 / feedback 28→30 / skills 32→33→32**（archify 装入 +1、architecture-diagram 退役 −1），8 页现态 token 全收（含非机检维度 `MEMORY.md` 索引 98→101 行 / skills 目录 34→35→34 / memory-index 计数口径戳 @08-29），`--check` 修后静默；memory-index **2 指针补登**（Harness 组 2→3 / Ohmybrain·文档工作流组 12→13）。② **dimension-C 漂移收口 2**：AUVProposal `15f3dba`(08-23)→**`b7d0921`(08-28)**（+4 commit：SPEC-032/033 文档管理体系 + 表 1.1 291 项转录 + 34 模板重建 + 齐套目录落 `ProjectManagement/` / SPEC-034 换能器规格书 + SPEC-035 整艇软件流程 V0.1→V0.4 / SPEC-036 舱段重排+能源口径全套出图 / SPEC-037 软件体系架构+板卡方案乙回归 / SPEC-038 v4 架构图入方案+能源章补全；dirty 10→4）+ Hub 自身行自指滞后 `6acc657`→`4de5ee8`。③ AUTO-GIT-SNAPSHOT `--gen` 重打 @08-29（36 仓）。④ **dimension S 重算 @08-29**：既有 🕸️ 天数 +5d；AUVSurvey 35d 新跨阈，同十六轮判「阻塞在用户非遗忘」注记不标（ImgSonarTwin 40d / EnvDataClassify 37d 同判）。⑤ **新 skill `archify` 登记**（JSON IR → 可交互自包含 HTML 图；schema 校验 + 原子交付 + spec/artifact 双 SHA-256 收据 + `visual-check` 恒报 pending；harness-resources 分组行 + 完整清单收口）。⑥ **审计十五验收口径交卷（本轮到期）**：query/promote **不再为零**——本 session 回答 archify 评估前实走 Hub query（[[../concepts/harness-engineering]] + [[../source-summaries/cocoon-ai-architecture-diagram]]），并命中「archify = cocoon v1.0 同血统续作」这一非平凡关联；随即**一行 promote ×2** 落 [[../concepts/skill-layered-resources]]（分层判据获同血统实证）+ [[../concepts/harness-engineering]]（Codex 引擎开源=第三方对照 + agentskills.io 开放标准）。**Hub 定位不降级**。⑦ **同轮用户批准后即办 3 项（原 surface → 已办）**：**(1) skill 同域冲突已裁**——`architecture-diagram` 退役（原件留存 `raw/architecture-diagram-generator/` 可复装），由 `archify` 取代，skills **33→32** 级联收口（conventions / dashboard / harness-resources 分组行+完整清单）+ [[../source-summaries/cocoon-ai-architecture-diagram]] 加继任注；**(2) 机检盲区第 2 例已根治**——`CANON_CHECKS` +1 条（harness-resources「本地 N 个 SKILL.md 完整清单」行，`--check -v` 验证 fire 正常，同 self-check 12 源头表补检先例）；**(3) Codex harness 正式 ingest 完成**——新建 [[../source-summaries/openai-codex-harness]]（crate 分域架构 + agentskills.io 开放标准 + 三引擎对照表）+ [[../source-summaries/tt-a1i-archify]]（四机制可移植清单 + 本机实测回执；顺带闭合 cocoon 页 04-17 留下的分层判据悬案），内容页 **110→112** / source-summaries 34→36。**Surface（仍待用户裁）**：④ raw/PPT 中安智能 pptx 仍未跟踪（08-18 已摄入 wiki；`draft/archify/` 工作台已被 `.gitignore:66 draft/` 覆盖，非待裁项）⑤ 前轮存量 surface 全未决（UWAcomm-pooltest worktree / `_impeccable_samples` / `DocProcess/papers` 口径 / MetalDinoForge·SlotForge·ObsidianStyleLab / **push 授权** Hub 两远程 +8）⑧ **同日续：跨引擎 skill 复用最小验证通过**——`mklink /J` 挂 `tech-requirements` 到 `~/.codex/skills/`（**实际路径非文档所写 `.agents/skills`**，codex-cli 0.144.0-alpha），`codex debug prompt-input` 前后对比注入条目 11→12 且路径穿透显示 `.claude` 原文件（单一事实源成立）；仅验「被加载」未验「被执行」，`paths:` 触发跨引擎必失效；页面总数 **110→112**；不新建 audit memory（第 17 轮，同 14/15/16 轮纪律））。**前轮 2026-08-24（入会审计（十六））**：距十五 6 天，机检基线全绿——`--check` 静默 / memory 97=1/28/65/3 零 CANON 债，本轮无计数级联。① **dimension-C 漂移收口 2+2**（dashboard 手写行）：UWAcomm `5df75c8`(08-03 codex 分支口径)→**main `b66c918`(08-23)**（三体制服务工程化收官：DSSS/FH-MFSK/S2C 服务 L1-L4 + C1-C5 校验 + spec/plan 归档用户终审通过，dirty=0，origin 落后 23）/ AUVProposal `58c44b8`→**`15f3dba`(08-23)**（+5 commit：SPEC-027~031 六舱段 12 号图 0820 贯通版+走线图重出+布置大挪移全链+物料清单核对版+W40 重号收口术语统一；dirty=10 含 SPEC-032 全生命周期文档管理规范+0824 交付材料包未提交）+ **手写行 stale ×2 收口**（DigitalTwinGuide/VisioForge 08-18 backup_push 首 commit 后 auto 表已新、dashboard 手写行漏刷——「部分登记」同型小例）+ Hub 自身行自指滞后刷新 `4898937`→`6acc657`；② **dimension S 重算 @08-24**：既有 🕸️ 天数 +6d，新踩 30d 阈 3 项均判「阻塞在用户非遗忘」注记不标（ImgSonarTwin 35d 等全文审 / AUVSurvey 30d 等型谱清单增删 / EnvDataClassify 32d 待 SPEC-001）；③ **Surface（待用户裁）**：worktree `UWAcomm-pooltest` 未登记（自检十三已 surface）/ `_impeccable_samples` 未登记 / `DocProcess/papers` 登记口径不一致（DocProcess CLAUDE.md 有列、根 CLAUDE.md 与 dashboard 状态表均无）/ MetalDinoForge·SlotForge·ObsidianStyleLab 存量未决 / push 授权（Hub 两远程 +6 + USBL_hw gitlab+6）；页面总数不变 110；不新建 audit memory（第 16 轮，同 14/15 轮纪律）；**附工作模式诊断五条**（元工作占比过高 / Hub 写多读少——query·promote=0 **十七轮交卷** / 备份纪律悬空 / 项目长尾无收尾出口 / 多 session 并行写 Hub 无互斥【本轮实证】，诊断「记录自检过剩、消费收尾不足」，详见 [[log]] 补记）。**同日（另 session 代登）dashboard git 快照例行刷新**：`dashboard_snapshot.py --gen` @08-24 15:08 重生成 AUTO-GIT-SNAPSHOT 区块——36 仓 HEAD/静默/dirty 刷新，UWAcomm 分支回 `main` `b66c918` / AUVProposal→`15f3dba` / EnvDataClassify 32d 新跨阈 🕸️?；业务叙事行未动，页面总数不变 110）。前轮 2026-08-18（**同日续 3 | 大脑激活：首次 memory→wiki 蒸馏 + 查询解放 + 一行 promote 标准（优化项 5）**：UWAcomm 04-23~05-16 十二条逐日 session memory 细读蒸馏 → 4 concept 页新增「实战结论」节（time-varying-channel / channel-estimation-and-equalization / doppler-estimation-methods / ofdm-and-otfs，每条一行结论+来源+日期）+ 12 条并入 `project_uwacomm_chronicle_2026H1` 编年条——**memory 108→97 / project 76→65**（工作记忆瘦身首刀，CANON 6 页级联 `--check` 静默）；query 三层协议从 llm-wiki skill 抽升全局 CLAUDE.md（解除「仅 wiki 路径触发」结构性死锁）；一行 promote 标准固化 conventions §6 + llm-wiki SKILL；验收=下下轮审计数 query/promote 条目，为零则降级 Hub 定位；页面总数不变 110。**同日续 2 | 基建落地（审计十五优化项 1+3）**：① dashboard git 事实层脚本化——`dashboard_snapshot.py` +`--gen`，全仓自动发现 36 仓快照写入 dashboard `AUTO-GIT-SNAPSHOT` 标记区块，手写行只留业务叙事（git 锚点漂移类「部分登记」根治）；② 本地单点仓双轨备份——新脚本 `backup_push.py`（scripts 24→25）：DigitalTwinGuide `4d9746e`/VisioForge `07f40cd` 首 commit 收口 + 16 仓预配内网 gitlab remote（内网不可达暂未 push，回内网 `--push` 一键）+ 21 仓 bundle 落 `Archive/git-backups/`（Patents 禁远程仅 bundle 轨，红线口径同步修正）；README 第 13 登记面补收（退役注 + 计数 + 指向 dashboard）；页面总数不变 110。**同日续 1 | dsh ingest + harness-engineering 空页填充**：用户确认摄入 **deepseek-ai/deepseek-harness**（DeepSeek 开源 agent harness，「一切皆插件」Cordis 插件树，2026-08-13 创建 5 天 151k star）→ `source-summaries/deepseek-ai-deepseek-harness`——架构要点（profile/bundle 分层 patch / turn-step 事件流 / session log 单一事实源 / capability seam / 分层 skill registry）+ 与本体系对照（引擎层 vs 知识工序层）+ 内网国产化候选（与 zhongan-llm-platform-ppt 结成「内网 AI 底座对标」组，双向互链）；**顺带修复**：`concepts/harness-engineering.md` 自 04-13 起为 0 字节空页（index 有条目、lint 未察觉）——实质填充（造 harness vs 用 harness 两层定义 + 跨引擎设计模式对照表）；内容页 109→110 / source-summaries 33→34。**本轮早前 入会审计（十五）+ DigitalTwin1plusN 退役 + ingest**：距上轮检阅 31 天最长间隙，用户显式要求审计。① **DigitalTwin1plusN 退役登记（ADR-041 体系首例）**——dimension-C sweep 发现工作区目录整体消失（本地 git 12 commit 无远程随之移除），用户确认系本人删除：活跃 30→29 / DocProcess×17→×16，root/Hub/DocProcess CLAUDE.md 删行 + 导航卡删除（32→31）+ dashboard 墓碑行 + system-overview/conventions 删行 + memory 退役注，历史记录（ADR-019/roadmap/log）逐字不动；② **8 处 dimension-C 漂移收口**（UWAcomm→`5df75c8` 08-03 / USBL_hw→`eb7252a` / SonarFOM +`9456931` / AUVProposal→`58c44b8` 08-16 / AUVSurvey→`6a79e02` / CoupledMultiOrder 整行翻新（`401b4b4`+智能艇群阶跃式协控+汇报 PPT 59 页 V5→V10）/ ImgSonarTwin 🔴→🟢 `5300532` / OceanEnvSupport→`15833b0`）；③ **CANON 级联收口**：memory 95→108 / feedback 24→28 / project 67→76（16 处 stale token）+ memory-index 14 指针补登（含 fieldkit bullet 漏列）；④ dimension S 重算：新标 🕸️ 5 项（UWAprojDoc/SonarSim/CooperativeASW/PaperTrans/UWCombatPlatform）+ 天数刷新 + Patents dirty=93 注；⑤ **ingest**：raw/PPT 中安智能大模型平台 pptx（36 页，内网 910B 部署对标参照）→ `source-summaries/zhongan-llm-platform-ppt`——内容页 108→109 / source-summaries 32→33；Surface：未登记 repo ×3（MetalDinoForge/ObsidianStyleLab/SlotForge）+ 两 remote 停 06-29 待 push 授权；不新建 audit memory（演化 [[feedback_ohmybrain_self_improvement]] 第 15 轮）。**前轮（2026-08-05）CoupledMultiOrder 派生登记**：派生 **CoupledMultiOrder** 🔒（DocProcess，多阶耦合方案文档，template-document，主交付物 docx 待定名，无依赖、**方向待用户细化**，ADR-040，**git 未 init** 待授权）——登记面派生当日全量，CANON 级联 活跃 29→30 / DocProcess×16→×17 / ADR ~039→~040；沿用 OceanEnvSupport「主交付物待定也先搭架子」先例；memory CANON 债 +1 留下轮入会自检；页面总数不变（projects/ 导航卡不计 content 页）。**前轮（2026-07-24）AUVProposal + AUVSurvey 双派生登记**：见 [[log]] 同日两条（活跃 27→29 / DocProcess×14→×16 / ADR ~037→~039，两仓 git init `5045bb3`/`544bc38`）。**前前轮（2026-07-23）EnvDataClassify 派生登记**：派生 **EnvDataClassify** 🔒（TechReq，环境数据分类：声速剖面/水文自动分类，template-engineering，MATLAB/Python，依赖 SonarFOM 🔒，ADR-037，git init `a7de7b2`）——登记面派生当日全量，CANON 级联 活跃 26→27 / TechReq×7→×8 / ADR ~036→~037；同批 SonarFOM 状态刷新为 🟢 活跃（水文五档分类 `7bbfadd` + SPEC-002 BELLHOP TL 链路 + ±40° 会聚区诊断）；memory CANON 债 +1 留下轮入会自检；页面总数不变（projects/ 导航卡不计 content 页）。**前轮（2026-07-21）SonarFOM 派生登记**：派生 **SonarFOM** 🔒（TechReq，声呐效能 FOM 品质因数表计算，template-engineering，MATLAB，依赖 SonarSim 🔒，ADR-036，git init `fb00819`）——登记面派生当日全量，CANON 级联 活跃 25→26 / TechReq×6→×7 / ADR ~035→~036；memory CANON 债 +1（`project_sonarfom_init`）连同 07-18 挂账留下轮入会自检；页面总数不变 108（projects/ 导航卡不计 content 页）。**前轮（2026-07-18 刷新）入会检阅（十四）**：CANON 级联收口 memory 87→95 / feedback 22→24 / project 61→67（8 页 16 处现态 token + `MEMORY.md` 88→96 行 + [[topics/memory-index]] 补登 8 条：feedback +2 / project +6，07-09 存量债清零）+ **Patents 状态刷新**——「无 git」已 stale（06-29 本地 git init `53bd96d` 仅本地无远程）且候选 3→6（+04 turbo-vamp 热启动信道估计 / 05 宽带数字直驱 / 06 无先验多模盲收），根 CLAUDE.md / Hub CLAUDE.md / dashboard / projects 卡 / MEMORY.md 五面收口；raw/PPT/ 未跟踪待处置；不新建 audit memory（演化 [[feedback_ohmybrain_self_improvement]] 第 14 轮）。**前轮 入会自检（十三·收尾自检十二未提交批次）**：对抗 workflow 抓 **calibration worktree pin 跨 3 anchor stale** `eae7080`→`c8adb32`（落后 7 commit，round-9「worktree pin 部分登记」漏传 conventions:180/根 CLAUDE.md:58/memory）已修 2 处（根文件待授权）+ surface ③ 经核实**自检十二代码已实装**（narrative 误标「待裁」，reconcile）+ dimension C **16 pin 全 match** 最干净一轮 + CANON 0 stragglers（memory 87 自洽）；不新建 audit memory（演化 [[feedback_ohmybrain_self_improvement]] 第 13 轮）。**前轮 入会自检（十二）**：**memory 86→87 / feedback 21→22 CANON 级联收口**——「审计后窗口」第 4 次复发（06-27 PPT session 新增 feedback `inplace-edit-no-version` 当日漏级联），8 页 ~20 处现态 token + 索引 87→88 行全收 + 新 memory 全登记面（MEMORY.md/memory-index）补登；全仓 git-HEAD sweep 17 项目 pin 全 match、Hub 自身行 `6f1bce1`→`7c82655` 自指滞后（gitlab even/github 落后 3）、dimension S 4 项 🕸️ @06-29 重算；33-agent 对抗 workflow——**验证者系统性误判 `@last-sync` 戳为冻结历史而过度 refute，主会话据 `--check` 仲裁器 + 亲核 CANON 源头表逐条裁定**，补 2 处 finder 漏；不新建 audit memory（演化 [[feedback_ohmybrain_self_improvement]] 第 12 轮）；**Surface**：未登记 repo×2 / dimension S 新跨阈×2 / --check 不覆盖 CANON 源头表。**前轮 入会自检（十一，2026-06-27）**：新日期 git-HEAD sweep 18 pin 全 match + PPT 评审 raw note 归档（CANON 零变动）。**入会自检（十）**：新 session git-HEAD sweep（28 仓含 worktrees）对照 18 pin——16 精确 match，对抗 workflow 独立重扫 **0 漏 / 0 误**；收口 2 处 dimension-C 漂移：UWCombatPlatform `30e428d`(暂停)→`3ce4928`(06-26 +3 commit 重启，六环节重构 + 6 平台路线 + AUV 统一 + 交接 Codex，「审计后窗口」第 3 次应验) + Hub 自身行 `7408107`→`4c822ff`(自检九已提交·自指滞后)；**仅 2 文件改动、页数与 CANON 计数不变（108 / memory 86 / 活跃 22）、不新建 audit memory**；**前轮同日** **入会自检（九）**：dimension C 14 主项目焦点 hash 全 match（对抗 workflow 另抓 3 处 worktree/状态次级漂移修正）+ DigitalTwin1plusN 05-25 交付后 32 天跨阈标 🕸️（+3 项 🕸️ 刷天数 + 旗舰 carve-out）+ **`dashboard_snapshot.py --check` 实装「活跃项目数 22 / DocProcess×11」项目登记机检**（导航卡−2 非项目卡 + conventions §9 行数 ground-truth，根治连续 7 轮「部分登记」project 计数盲区，自检 7/8 surface 未根治本轮落地）+ frontmatter 去重；**仅文件改动、页数与 CANON 计数不变（108 / memory 86 / 活跃 22）、不新建 audit memory**；前轮 2026-06-25（**入会自检（八）**：审计后两笔操作 CANON 复核——登记矩阵全 present（连续 7 轮「部分登记」未续）+ 4 处现态计数 stale 收口（workflow-glossary「109 个 .md」→108 / memory-index「USBL（1）」→（2）/ root CLAUDE.md DocProcess 表补 PaperTrans 10→11 / memory-stack「auto-memory 16 条」→86），均对齐已正确 canon，**页数与各 CANON 计数不变**；不新建 audit memory 避免级联；**ingest**：立项论证报告模板脱敏摄入 `source-summaries/lixiang-lunzheng-report-template`——内容页 107→108 / source-summaries 31→32 / 总文件 109→110；**起新项目 UWCombatPlatform**（水下作战平台建设方案+报价，template-document，活跃 21→22 / DocProcess×10→×11 / memory 85→86 / project 60→61 / ADR-032）；**入会自检（七）**：UWAcommTrial 项目补登——06-24 派生当日「部分登记」漏 8 面，收口 dashboard / system-overview 实例表 / ADR-031 / roadmap / memory-index / log / auto-memory，活跃 20→21、DocProcess×9→×10、memory 84→85 / project 59→60、ADR ~030→~031；页面总数不变 107，projects/ 导航卡不计入 content 页）；前轮 2026-06-24（入会自检（六）：PaperTrans 项目补登——06-15 派生当日漏登 dashboard/system-overview/memory-index/ADR-029（DocProcess×8→×9、活跃 19→20）+ USBL_hw 进展刷新（→`c7c07da`/71 commit/已建 gitlab 远程）+ CANON 级联修复 skills 31→32、memory 81→83/project 56→58、ADR range ~028→~029 + CooperativeDetection/PaperReview 标 🕸️；**+ ppt-master 采纳（Plan A/ADR-030）**：FIELDBOOK→`fieldbook` brand/deck 模板 + AnthropicPPT 降级 + memory 83→84/project 58→59、ADR ~029→~030（ppt-master = 第三方 vendored 不计活跃项目）；上批 2026-06-15 FieldKit 派生（ADR-027/028）；页面总数不变 107，projects/ 导航卡不计入 content 页）
> 页面总数：112

## Concepts（概念页）

- [anti-patterns](concepts/anti-patterns.md) — 跨项目反模式合集，按阶段分类（Research / Build / RCA / Promote / 全阶段），feedback_* memory 28 条提炼（2026-05-24 新建）
...
- [memory-index](topics/memory-index.md) — `~/.claude/projects/D--Claude/memory/` 99 条 auto-memory 按类型 + 主题聚合索引（2026-05-24 新建）
...
- [deepseek-ai-deepseek-harness](source-summaries/deepseek-ai-deepseek-harness.md) — DeepSeek 开源 agent harness（dsh）：「一切皆插件」Cordis 插件树 + turn/step 事件流 + 分层 skill registry，Claude Code 概念面的开源镜像；内网国产化「自建」路线候选（2026-08-18 ingest）
- [zhongan-llm-platform-ppt](source-summaries/zhongan-llm-platform-ppt.md) — 中安智能大模型及智能应用平台厂商 PPT（36 页）：水下任务场景四层平台 + 辅助开发/低代码工作流/RAG/决策 Agent 四能力 + 内网 2×8 卡昇腾 910B 部署与国产模型选型；内网国产化对标参照
- [openai-codex-harness](source-summaries/openai-codex-harness.md) — OpenAI 2026-08-20 开源 Codex 引擎本体（app-server/exec/SDK，Apache-2.0，Rust）：crate 分域架构 + **skills 走 agentskills.io 开放标准**（与本工作区 32 skill 结构同构，存在跨引擎复用可能）；harness 对照组第三员（2026-08-29 ingest）
- [tt-a1i-archify](source-summaries/tt-a1i-archify.md) — JSON IR + schema 校验 + 原子交付双 SHA-256 收据的出图 skill（27.6k★ MIT，5 图型 × 4 预设 → 可交互 HTML）；即 cocoon v1.0 同血统续作，已装入并取代之（2026-08-29 ingest + 本机实测）
