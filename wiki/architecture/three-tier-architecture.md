...
2. **project 不直接更新 core**：项目的新工作流先回流 Hub，由 Hub 评估后才更新 core template/
3. **core 不主动派生项目**：派生是 user 在新项目时主动 `cp -r ohmybrain-core/template/`
...
├── projects/.../memory/ → auto-memory 跨会话恒久（60+ 条）
...
- 不需要 project 派生时重新 cp template/，已运行的项目立即生效
