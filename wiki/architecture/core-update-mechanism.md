...
「Hub = 大脑」需要主动维护 ohmybrain-core 的三套模板（`template-engineering/`、`template-document/`、`template-tool/`），让成熟模式按项目类型下沉。**当前 = 手动同步 + `/sync-to-core` 命令辅助**。
...
```markdown
### 下沉建议
- core/template-engineering/.claude/skills/<name>.md 添加（已在 UWAcomm / USBL 验证）
```
...
如 2026-05-24 三仓哲学澄清，可能触发 `template-*/CLAUDE.md` 中"Hub 大脑"描述更新。
...
type: concept
core_candidate: true
core_target: template-engineering/.claude/skills/lint_wiki.md
---
```
...
<!-- candidate-for-core: template-engineering/.claude/skills/lint-wiki.md -->
```
...
2. 对每个候选条目：读 Hub wiki 详细页 + 比对对应的 ohmybrain-core/template-*
...
4. **按模板归属下沉**：engineering 专属内容不强加给 document/tool；document 的私人约束不强加给公开工具；通用内容才三模板同步
5. **修改 core 必须 git commit**：不要本地修改不 commit（造成新派生项目用错版本）
...
- **2026-06-09** 同步三模板口径：core target 必须指向具体 `template-*`
