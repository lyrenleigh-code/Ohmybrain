...
ohmybrain-core 的 `template-*/workflows/` 定义独立但相互触发的工作流闭环。**两者都在 project 内执行**，Hub 不参与具体闭环，但 Hub 接收 `knowledge.promote` 的产出。
...
来源：`ohmybrain-core/template-engineering/workflows/engineering/`

| 步骤 | 动词 | 触发 | 工具 / 命令 | 产出 |
|------|------|------|-----------|------|
| **01** | `spec` | 新任务开始（query 完成、知识 ready） | 写 `specs/active/<slug>.md`（目标 / 范围 / verification） | spec.md |
| **02** | `plan` | spec 复杂或跨多文件 | 写 `plans/active/<slug>.md`（分阶段、依赖、风险） | plan.md |
...
- **2026-06-09**：路径口径同步为三模板 `template-*`，计划目录统一为 `plans/active/`
