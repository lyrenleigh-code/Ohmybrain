...
**补记 | 工作模式诊断（用户要求审计工作模式本身，同 session）**：五大问题——① **元工作占比过高**：16 轮审计收口的多为登记面自身漂移，一次项目动作过 ~12 登记面超人工一致性带宽，「审计劳动在补贴登记设计」；方向 = 继续收窄手写面（沿 `--gen` 路线），非加密审计。② **Hub 写多读少**：审计十五立的验收口径「下下轮审计数 query/promote 条目，为零则降级 Hub 定位」——十五→十六期间 query/promote 为 **0**，**下轮（十七）交卷**。③ **备份纪律悬空**：push 停 06-29（本地领先 7）/ Patents 无远程 dirty=93×56d / ObsidianStyleLab 无远程——`backup_push.py` 工具就位、执行悬空（DigitalTwin1plusN 12 commit 丢失是已发生事故非假设）。④ **项目长尾无收尾出口**：🕸️9 + 阻塞 5 近半停等；派生成本趋零而退役仅被动首例（ADR-041），缺主动「冻结/归档」轻量态。⑤ **多 session 并行写 Hub 无互斥**：本轮实证——index/log 被另 session 并行写入（--gen 代登条目），本 session Edit 冲突后手动合并；写权锁协议只覆盖项目仓，Hub 登记面是无约定共享热点。**诊断一句话：记录自检过剩、消费收尾不足**——下一步优化 = 更少手写面 / 被执行的备份 / 被使用的知识 / 被关闭的项目。连同 B 批次 5 项 surface 留用户回访执行。
