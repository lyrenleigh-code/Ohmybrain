...
>
> 全景计数（@2026-06-15）：本地 skills **34 个目录**（32 含 SKILL.md）/ 全局 agents **55 个 .md** / rules **15 个目录** / MCP **6 个 server**。
...
### 工作区级 Hooks (2 个，2026-06-10)

脚本托管 Hub `scripts/`（git 跟踪），**仅注册 `D:/Claude` 会话根** `settings.json`（scope=全工作区，非 Hub 守卫，不计入上表 8）：

| Hook | 类型 | 触发时机 | 作用 |
|------|------|---------|------|
| `check_push_readme.py` | 🔴 阻断 | PreToolUse Bash | `git push` 前检查 README* 随之更新，`SKIP_README=1` 前缀逃生 |
| `calendar_reminder.py` | 🟡 提醒 | Stop | 今日 `calendar/YYYY-MM-DD *.md` 未建则提醒（4h 节流） |
...
| 层 | 计数 | 来源 |
|----|------|------|
| **本地层** | **34 个目录**（32 含 `SKILL.md`，2 个为工件目录无 SKILL.md） | `~/.claude/skills/` 实际落盘 |
| **注入层** | **90+ 个**（含 `ecc:*` 前缀 plugin / marketplace） | SessionStart 注入 plugin skill list 叠加后 |

> ⚠️ 不可裸写「90+ 个全局 skill」——那是注入层叠加 ecc plugin 后的数；本地实体只有 34 目录 / 32 SKILL.md。

### 2.1 本地 Skills（`~/.claude/skills/`，32 个 SKILL.md）

按域分组（共 32 个 SKILL.md，另 2 个目录 `jy-pricing-workspace` / `learned` 是工件目录，无 SKILL.md）：
...
SessionStart 注入的 plugin / marketplace skill，叠加在本地 32 个之上后总数达 **90+**。`ecc:` 前缀（Everything Claude Code）覆盖：
...
项目/.claude/rules/* 按路径加载（visit wiki/ → wiki.md）
  ↓
本地 32 个 SKILL.md / 55 个 agents / 6 个 MCP 按需调用（关键词 / 显式 / 路径）
  ↓
...
- [[../architecture/decision-log]] — ADR-001~029（ADR 章节，非独立文件）
