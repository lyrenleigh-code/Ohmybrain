...
> **计数口径（@2026-06-10）**：auto-memory 共 **80 个**条目文件（外加 `MEMORY.md` 索引本身 = 目录 81 个 `.md`）。分布：user **1** / feedback **21** / project **55** / reference **3**。下方各类型标题计数与此严格一致。
>
...
> 注：`project_uwacomm`（含 13 条 session 条）+ `project_uwacomm_usbl`（含 13 条）+ `project_uwaprojdoc`（含 3 条）等高频项目的多日 session 条目均计入上方分组，加 Tools 系 7 + VisioForge/CooperativeASW/SonarSim/USBL_hw 各 1 + Ohmybrain Hub session 2 条（2026-06-09 / 2026-06-10），合计 55。
...
- **last-sync: 2026-06-10**（计数已对齐 CANON：feedback 21 / project 55 / reference 3 / user 1 / 总 80）
