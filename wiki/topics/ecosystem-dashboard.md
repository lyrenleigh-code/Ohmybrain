...
| **CooperativeDetection** 🔒 | 🕸️ 待回访 | 4 专题 12 课题方案（2026-05-08 派生）；HEAD `6113b96`（05-09，51 天未动，dirty=0） | `D:\Claude\DocProcess\CooperativeDetection` |
| **PaperReview** 🔒 | 🕸️ 待回访 | 学位论文外审（中文）；2026-05-09 派生，HEAD `b3568f6`（51 天无新 commit，工作树 dirty=7——可能在评中，待回访确认） | `D:\Claude\DocProcess\PaperReview` |
| **DigitalTwinGuide** 🔒 | 🕸️ 停滞 | 数字孪生实施指南方法论；2026-05-13 首版宋体 docx + 4 步 pandoc pipeline，docx 仍 stage 未 commit（dirty=65）；**init 起 master 无 commit，停滞约 47 天，待回访** | `D:\Claude\DocProcess\DigitalTwinGuide` |
| **DigitalTwin1plusN** 🔒 | 🕸️ 待回访 | 「1+N」水下集群数字孪生体系方案（2026-05-25 派生）；P1-P11 概念决议（P11 暂缓）+ 3 spec；**同日（05-25 晚）v0-v5 完整可研报告 docx 落地（4 步 pandoc pipeline，报价表按 V1.0 格式），HEAD `234eb11`→`c866bb7`（12 commit，本地无远程）**（auto-memory 待回填同日 docx 延续）；**自 05-25 交付后 35 天无新 commit（dirty=1），入会自检（九）@2026-06-26 跨 30 天阈值标 🕸️、自检十二 @2026-06-29 重评 35 天，待回访确认收尾 / 暂停** | `D:\Claude\DocProcess\DigitalTwin1plusN` |
...
| **ImgSonarTwin** 🔒 | 🔴 刚派生 | 图像声呐数字孪生方案文档（2026-07-18 派生，template-document，依赖 DigitalTwinGuide/DigitalTwin1plusN，ADR-035）；主交付物《图像声呐数字孪生方案》（待定稿名，docx）；脚手架就位（SOP §1 派生 + 占位符全清 + §6 验证全过），撰写未启动，待 raw/ 资料摄入 + SPEC-001 章节大纲；本地 main 无远程 | `D:\Claude\DocProcess\ImgSonarTwin` |
| **AUVProposal** 🔒 | 🟢 活跃 | AUV 项目立项论证（2026-07-24 派生，template-document，依赖 AUVSurvey，手动模式，ADR-038）；**同日 SPEC-001 confirmed**（三轮讨论：协同探测型 1 t×3 艘/2000 m/≥10 km/≥24 h、四分系统组成、主被动结合、dock 回收不充电、组网算法出本期、申报书体 10 页、一年周期、一体化设计硬约束）+ **02-draft 初稿七章**（§6 研究基础用户指示占位不写，5 图占位待 flowgen）+ Codex 交接单（审校任务）；下一步=用户终审 + §6 素材 + 图件 + docx；本地 main `5045bb3` 无远程 | `D:\Claude\DocProcess\AUVProposal` |
| **AUVSurvey** 🔒 | 🟢 活跃 | AUV 广泛调研（2026-07-24 派生，template-document，无依赖、下游 AUVProposal，手动模式，ADR-039）；**同日 SPEC-001 confirmed**（D1-D4：全谱覆盖/15~25 型/联网检索/精干版 30~50 页）+ **首轮 6 路并行调研落 wiki/topics 6 专题页**（国外/国内型谱各 16 型 + 声呐/能源/导航控制/总体设计，lint/index 6 页 ✓）+ Codex 交接单（核验+起草任务）；门槛项=型谱清单用户增删（32→15~25）→ 02-draft；本地 main `544bc38` 无远程 | `D:\Claude\DocProcess\AUVSurvey` |
...
| 项目 | 状态 | 当前焦点（最近 session / 锚点） | 路径 |
|------|------|---------|------|
| **Patents** 🔒 | 🟡 候选 | 私密专利交底书工作区（**本地 git**，2026-06-29 init `53bd96d`，仅本地无远程）；6 候选交底书：iusbl-jacobian / otfs-spread-pilot / usbl-cage5-3d-hybrid-doa / turbo-vamp-warmstart / wideband-digital-direct-drive / no-oracle-multimode-blind-rx | `D:\Claude\Patents` |
...
| Memory 条目 | **95** | 4 类：user 1 / feedback 24 / project 67 / reference 3（见 [[memory-index]]；最新含 CC 方法论 PPT v16 + UWAcomm Linux 工程化核验 + CommSimSupport / OceanEnvSupport init + Patents 06-29 + 2026H1 个人总结 条目） |
...
- [[memory-index]] — auto-memory 95 条目索引（本页 session 锚点来源）
