...
- **论证类文档的链路预算用声纳方程直接算接收 SNR**（SL − TL − NL + DI，参数取本项目实际的换能器、频段、阵型与安装位置），不凭经验给 SNR、不沿用仿真平台默认值；仿真门限与声纳方程结果对不上时先查自己的参数设置。— AUVProposal 通信论证 2026-08-12（用户：「接收处的信噪比可以通过声纳方程去计算」）；2026-09-19 会话记录扫描补登
