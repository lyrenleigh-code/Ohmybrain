...
- [x] **A. 用户执行（2026-09-19 已完成并验证：AcousticLiteracy 目录新开会话加载主库、索引读到最后一行）**：`~/.claude/settings.json` 加 `"autoMemoryDirectory": "~/.claude/projects/D--Claude/memory"`，让所有项目和 worktree 会话共用主库（Claude 自改 settings 被权限 classifier 拦截）；加后在任一子项目新开会话验证
